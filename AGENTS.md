...
- **Sibling project (2026-07-24, Langdock site — Creative Health client case study on /systems, PLANNED + BUILT + DEPLOYED)** — `/ce-plan` then build: a new `/systems` deep-dive (after the Ninety2 deep-dive) telling the story of the **creative-health-demo ("Pip's Peaks")** client project, a Google-ADK creative "trading desk" (one verdict per ad creative: Scale/Sustain/Fix/Retire) forked into a public demo on synthetic data. Direction A chosen from a 3-direction mockup Artifact; two REAL live-tool screenshots (decision map + diagnosis/action-plan/chatbot) in `public/images/`; facts in new `src/data/creative-health.ts`; new `CreativeHealthDeepDive.tsx`. **Public-safe** — real client + infra masked as "Pip's Peaks", enforced by `creative-health.test.ts` (char-code banned tokens + infra regexes, mirrors the source scrub-gate). Gates green (lint 0, 401 Vitest, build clean); Chrome + WebKit verified; MERGED to main (`349bce8`), pushed, deployed (`langdock-application-kvnb26dus`), live-verified. Plan at `docs/plans/2026-07-24-001-feat-creative-health-systems-section-plan.md`. No web-app-resume source changes. **Lesson: chrome-devtools CLI drives a SEPARATE Chrome (not the user's logged-in browser); `&nearr;` renders literally in SWC JSX (use lucide `ExternalLink`).** Detail in auto-memory (`langdock-application-site.md`).
- **Sibling project (2026-07-24, Langdock site — Safari/iPhone fix + mobile nav menu, DEPLOYED)** — Fixed what broke on Dominik's iPhone. Two causes: (1) production was serving an UNMERGED Codex branch (`codex/premium-imagery`, 3 hero/section images `vercel --prod`'d but never merged to main; found because the live images were in no committed state of main); (2) those `next/image fill` images sized ONLY by CSS `aspect-ratio` **collapse to 0px height in WebKit/Safari** (fine in Chrome), so they were invisible on iPhone. Fixed with an in-flow padding-ratio spacer via a `ratio` prop (`8ac9555`); merged the branch to main. Then added a **mobile hamburger menu** (new `MobileNav.tsx`, `5207106`) so the full DB × Langdock logo + all links (incl. "Why Langdock + Dominik") reach mobile without re-triggering the old 390px nav overflow. HEAD `5207106`, pushed, deployed + live-verified in real Safari (388 Vitest, lint 0, build clean). **Lesson: chrome-devtools is Blink and CANNOT reproduce Safari/iOS bugs — reproduce "broken on iPhone" with `playwright-cli --browser=webkit`.** No web-app-resume source changes. Detail in auto-memory (`langdock-application-site.md`).
- **Sibling project (2026-07-22/23, Langdock application site — BUILT, REVIEWED, POLISHED, DEPLOYED)** — Executed `docs/plans/2026-07-22-001-feat-langdock-application-site-plan.md` to Definition of Done in one `/goal` → `/ce-work` session, building the fifth and most important application site in its own private repo `Ninety2UA/langdock-application` (live at **https://langdock.dbenger.com**, permanent noindex). All 15 units (Langdock design system, Cursor-grammar console with 10 authored prototypes, Kimi K3 concierge, Agentic Systems page with Excalidraw figures, hosted ninety2 walkthrough copy, OG/QA docs), then a full `/ce-code-review` (7 reviewers + codex cross-model; 19 fixes applied, `4759fb6`) and a chrome-devtools/playwright 5/5 pass (`0960ef1`: two-column hero with console glimpse, real 390px mobile-nav overflow fixed). 28 commits, 283 Vitest green, lint 0, build clean, deployed + live-verified. Only the send + copy approval remain Dominik's. **No web-app-resume source changes** (only this repo's plan doc). Detail in auto-memory (`langdock-application-site.md`).
