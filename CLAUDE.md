...
- **Animations:** CSS scroll-triggered via IntersectionObserver + `data-animate` attributes, hero canvas skill-word constellation (v2: Poisson-disk layout, Lissajous oscillation, hover proximity highlight, ambient connection lines)
...
## Session Continuity
- **Latest work** — Reworked hero canvas into interactive skill constellation (commit `396c47a`). All 27 skill words now render at once (was ~6 of 27) via Poisson-disk layout on desktop + 2-col stagger on mobile. Separate `WordLabel` class (home position + Lissajous oscillation + mouse field + entrance stagger) and `Particle` class (background dots only). Proximity highlight (brand-500→brand-600, scale 1.32×, push 210px radius), always-on ambient constellation lines, periodic pulses on desktop+mobile, DPR-scaled canvas, IntersectionObserver pause, `prefers-reduced-motion` respected, `aria-hidden` on canvas. Lives in FEATURE 5 block of both `public/site.html` and `docs/index.html` (kept byte-identical).
- **Deployed** — `396c47a` live at https://dbenger.com (Vercel build 22s, deploy 44s). Manual deploy via `npx vercel --prod` when Git auto-deploy doesn't trigger.
